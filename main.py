...
async def root(code: str, state: str, background_tasks: BackgroundTasks):
    if (state not in app.states):  # simple check to see if request is from spotify
        ...
        if (access_token_request.status_code == 200):
            ...
            app.states[state][2] = getUserInfo(
                app.states[state][0])["username"]
    ...
    try:
        while True:
            ...
            if (data.startswith('!state')):
                ...
                weightSongs(app.states[state][2], app.states[state][0])
                await websocket.send_json({"type": "message", "message": f"Your playlist was created"})
            else:
